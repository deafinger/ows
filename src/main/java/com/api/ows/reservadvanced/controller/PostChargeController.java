...
/**
 * @Class PostChargeController
 * @Description : 결제항목 추가 Controller 
 * @
 * @ 수정일      	     수정자           수정내용
 * @ ---------  	 ---------   	-------------------------------
 * @ 2021. 5. 21.     서민재     		최초생성
...
@Slf4j
@RestController
@RequestMapping(path = "/reserAdvanced/postCharge")
public class PostChargeController {
	@Autowired
	ValidationErrorsMessage mes;
	
	@Autowired
	PostChargeService service;
	
	@PostMapping(path = "/request")
	public ResponseEntity<?> postChargeRequest(@Valid @RequestBody PostChargeReqVO param,Errors errors) throws Exception{
		log.info("PATH :/reserAdvanced/postCharge/request");
		if(errors.hasErrors()) throw new BadParameterException(mes.getExceptionMessage(errors)); 
		return ResponseEntity.ok().body(service.doPostCharge(param));
	}
}
